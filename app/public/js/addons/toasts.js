...
/** Import dependences */
$('body').append(
    $('<link rel="stylesheet" />').attr('href', '/css/addons/toast.css'),
    '<div class="jq-toast-wrap"></div>'
);

let element = $('.jq-toast-wrap');
 ...
 * @param text
 * @param icon
 * @desc icon : info, error, success, warning
 *
 */
function newtoast(title, text, icon) {
...
function resetToastPosition() {
    if (element !== undefined) {
        element.removeClass('bottom-left bottom-right top-left top-right mid-center');
        element.css({"top": "", "left": "", "bottom": "", "right": ""});
    }
}